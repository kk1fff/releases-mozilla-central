                           ...
                           const nsAString& aMessage, jsval* aRequest)
{
  ...
  rv = nsContentUtils::WrapNative(aCx, aGlobal, domRequest, aRequest);
  if (NS_FAILED(rv)) {
                              ...
                              const PRUnichar* aData)
{
  if (!strcmp(aTopic, kSmsReceivedObserverTopic)) {
    DispatchTrustedSmsEventToSelf(aTopic, RECEIVED_EVENT_NAME, aSubject);
    return NS_OK;
  }

  if (!strcmp(aTopic, kSmsSendingObserverTopic)) {
    DispatchTrustedSmsEventToSelf(aTopic, SENDING_EVENT_NAME, aSubject);
    return NS_OK;
  }

  if (!strcmp(aTopic, kSmsSentObserverTopic)) {
    DispatchTrustedSmsEventToSelf(aTopic, SENT_EVENT_NAME, aSubject);
    return NS_OK;
  }

  if (!strcmp(aTopic, kSmsFailedObserverTopic)) {
    DispatchTrustedSmsEventToSelf(aTopic, FAILED_EVENT_NAME, aSubject);
    return NS_OK;
  }

  if (!strcmp(aTopic, kSmsDeliverySuccessObserverTopic)) {
    DispatchTrustedSmsEventToSelf(aTopic, DELIVERY_SUCCESS_EVENT_NAME, aSubject);
    return NS_OK;
  }

  if (!strcmp(aTopic, kSmsDeliveryErrorObserverTopic)) {
    DispatchTrustedSmsEventToSelf(aTopic, DELIVERY_ERROR_EVENT_NAME, aSubject);
    return NS_OK;
  }
