...
function createDomMessageFromRecord(aMessageRecord) {
  if (DEBUG) {
...
function createThreadFromRecord(aThreadRecord) {
  if (DEBUG) {
    debug("createThreadFromRecord: " + JSON.stringify(aThreadRecord));
  }
...
MobileMessageDatabaseService.prototype = {
  ...
  saveRecord: function saveRecord(aMessageRecord, aAddresses, aCallback) {
    ...
    function notifyResult(rv) {
      ...
      let domMessage = createDomMessageFromRecord(aMessageRecord);
  ...
  setMessageDelivery: function setMessageDelivery(
    ...
    function notifyResult(rv) {
      ...
      let domMessage = createDomMessageFromRecord(messageRecord);
...
ResultsCollector.prototype = {
  ...
  /**
   * Queue up passed id, reply if necessary. 'id' = 0 for no more results,
   * negtive for errors and valid otherwise.
   */
  collect: function collect(txn, id) {
    if (this.done) {
      return false;
    }
...
IntersectionResultsCollector.prototype = {
   ...
   * |realCollect|. Return true if it is still possible to have another match.
   */
  collect: function collect(contextIndex, txn, id, timestamp) {
    ...
    if (!id) {
      ...
      for (let i = 0; i < contexts.length; i++) {
        if (!contexts[i].done) {
          // Don't call |this.realCollect| because |context.results| might not
    ...
    return this.realCollect(txn, id, timestamp);
  },
...
UnionResultsCollector.prototype = {
  ...
  collect: function collect(contextIndex, txn, id, timestamp) {
    ...
    for (let i = 0; i < tres.length; i++) {
      this.realCollect(txn, tres[i].id, tres[i.timestamp]);
    }
    this.realCollect(txn, 0, 0);
...
GetMessagesCursor.prototype = {
  ...
  getMessageTxn: function getMessageTxn(messageStore, messageId) {
    ...
    getRequest.onsuccess = function onsuccess(event) {
      ...
      let domMessage = createDomMessageFromRecord(event.target.result);
...
GetThreadsCursor.prototype = {
  ...
  getThreadTxn: function getThreadTxn(threadStore, threadId) {
    ...
    getRequest.onsuccess = function onsuccess(event) {
      ...
      let thread = createThreadFromRecord(event.target.result);
